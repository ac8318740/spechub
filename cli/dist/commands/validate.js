...
export function register(program) {
    program
        ...
        if (name) {
            ...
            const specDir = join(root, SPECHUB_DIR, SPECS_DIR, name);
            if (existsSync(changeDir))
                results.push(validateChange(root, name, strict));
            if (existsSync(specDir))
                results.push(validateSpec(root, name, strict));
            if (results.length === 0) {
            ...
            if (doChanges) {
                for (const c of listChanges(root))
                    results.push(validateChange(root, c, strict));
            }
            if (doSpecs) {
                for (const s of listSpecs(root))
                    results.push(validateSpec(root, s, strict));
            }
        }
        if (opts.json) {
