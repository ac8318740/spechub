...
var SPECHUB_DIR, CHANGES_DIR, SPECS_DIR, ARCHIVE_DIR, CONFIG_FILE, GLOBAL_CONFIG_DIR, GLOBAL_CONFIG_FILE, GLOBAL_DATA_DIR;
...
function validateSpec(root, name, strict) {
  ...
  const specDir = join12(root, SPECHUB_DIR, SPECS_DIR, name);
  const specFile = join12(specDir, "spec.md");
  if (!existsSync11(specFile)) {
    result.valid = false;
    result.errors.push("Missing spec.md");
    return result;
  }
  const content = readFileSync6(specFile, "utf-8");
...
function register8(program3) {
  program3.command("validate").description("Validate changes and/or specs").argument("[name]", "specific item to validate").option("--all", "validate everything").option("--changes", "validate all changes").option("--specs", "validate all specs").option("--strict", "strict validation").option("--json", "output as JSON").action((name, opts) => {
    ...
    if (name) {
      const changeDir = join12(root, SPECHUB_DIR, CHANGES_DIR, name);
      const specDir = join12(root, SPECHUB_DIR, SPECS_DIR, name);
      if (existsSync11(changeDir)) results.push(validateChange(root, name, strict));
      if (existsSync11(specDir)) results.push(validateSpec(root, name, strict));
      if (results.length === 0) {
        console.error(source_default.red(`'${name}' not found as a change or spec.`));
        process.exit(1);
      }
...
import { existsSync as existsSync12, readFileSync as readFileSync7, writeFileSync as writeFileSync3 } from "node:fs";
function readGlobalConfig() {
  if (!existsSync12(GLOBAL_CONFIG_FILE)) return {};
  return JSON.parse(readFileSync7(GLOBAL_CONFIG_FILE, "utf-8"));
}
...
var pkg = JSON.parse(
  readFileSync8(join13(import.meta.dirname, "..", "package.json"), "utf-8")
);
