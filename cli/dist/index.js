...
var import_yaml2, COUNT, FRONTEND_VERIFICATION_KEY, PROJECT_KEYS, PROJECT_KEY_LIST, PERCENTAGE, BYTE_ORDER_MARK, FS_REASONS, UTF8, PROJECT_KEY_DEFAULTS;
...
async function browserModeWorks(mode, project) {
  ...
  return {
    ok: binary !== void 0,
    detail: binary ? `found ${binary} on PATH` : `no Chromium or Chrome binary on PATH (looked for ${CHROMIUM_BINARIES.join(", ")})`
  };
}
async function checkDeclaredBrowserModes(report, config, project) {
...
function writeAck(args) {
  ...
  writeAtomically(ackPath(file), `${JSON.stringify(record2, null, 2)}
`);
  return record2;
}
...
function isSidechain(record2) {
  return record2.isSidechain === true;
}
function isDelivery(record2, token) {
  if (record2.type !== "queue-operation" || record2.operation !== "remove") return false;
  const content = record2.content;
  if (typeof content !== "string") return false;
  return content.startsWith(CROSS_SESSION_PREFIX) && content.includes(token);
}
function isTurnBoundary(record2) {
  return record2.type === "assistant" && record2.message?.stop_reason === "end_turn";
}
function findSendMessage(record2) {
  if (record2.type !== "assistant") return void 0;
...
function findTranscriptAck(record2, options) {
  const sent = findSendMessage(record2);
  if (sent && (!sidecarIsAuthoritative(options) || sent.decision !== null)) return sent;
  return options.fresh ? findTextAck(record2) : void 0;
}
...
function isEngagement(record2, file) {
  if (record2.type !== "assistant") return false;
  const content = record2.message?.content;
...
function scanFromAnchor(records, anchor, options) {
  ...
  for (let i = anchor; i < records.length; i += 1) {
    ...
    if (isSidechain(record2)) continue;
    if (!scan.engaged) scan.engaged = isEngagement(record2, options.file);
    if (scan.ack !== void 0) continue;
    scan.ack = findTranscriptAck(record2, options);
    if (scan.ack === void 0 && isTurnBoundary(record2)) scan.boundaries += 1;
  }
  return scan;
}
...
function resolveAnchor(records, options) {
  ...
  return records.findIndex((record2) => !isSidechain(record2) && isDelivery(record2, token));
}
...
function parseVocabulary(markdown) {
  ...
  markdown.split("\n").forEach((line, index) => {
    ...
    if (cells.length !== VOCABULARY_COLUMNS) {
      warnings.push({
        line: index + 1,
        message: `Dropped a malformed vocabulary row: expected ${VOCABULARY_COLUMNS} columns, found ${cells.length}.`
      });
      return;
    }
...
function readyVocabulary(vocabulary) {
  const compiled = vocabulary.map(
    (entry) => isCompiled(entry) ? entry : compileVocabulary([entry])[0]
  );
  return compiled.sort((a, b) => b.avoid.length - a.avoid.length);
}
function columnAt(text, index) {
  let column = 1;
...
function maskInlineCode(line) {
  return line.replace(/``[\s\S]*?``/g, (match) => " ".repeat(match.length)).replace(/`[^`]*`/g, (match) => " ".repeat(match.length));
}
...
function frontmatterEnd(rawLines) {
...
function pipelessTableLines(rawLines) {
...
function classify(rawLines) {
  ...
  for (let index = 0; index < rawLines.length; index++) {
    ...
    const content = marker ? maskInlineCode(withoutQuote).slice(marker[0].length) : maskInlineCode(withoutQuote);
    lines.push({
      line: number,
      text,
      content,
...
function checkVocabulary(line, vocabulary) {
  const findings = [];
  const { text } = line;
  ...
  for (const entry of vocabulary) {
    ...
    while ((match = entry.pattern.exec(text)) !== null) {
      ...
      findings.push({
        ...
        message: entry.section === "words" ? vocabularyMessage(entry, match[0]) : markMessage(entry)
      });
    }
  }
  return findings;
}
function checkEmoji(line) {
  const findings = [];
  const { text } = line;
  for (const match of text.matchAll(EMOJI)) {
    const code = match[0].codePointAt(0);
    if (code === void 0 || LEGAL_MARKS.has(code)) continue;
    findings.push({
      line: line.line,
...
function checkLine(line, vocabulary) {
...
function endsSentence(joined, end, terminator) {
...
function lintProse(text, vocabulary) {
  if (text === "") return [];
  ...
  if (unterminated !== null) {
    findings.push({
      ...
      message: `The rest of the file was skipped as code: unterminated code fence opened at line ${unterminated.line}.`
    });
  }
...
var init_prose = __esm({
  "src/lib/prose.ts"() {
    ...
    EMOJI_PART = "\\p{Extended_Pictographic}(?:\\uFE0F|[\\u{1F3FB}-\\u{1F3FF}])*";
    EMOJI = new RegExp(`${EMOJI_PART}(?:\\u200D${EMOJI_PART})*`, "gu");
    LEGAL_MARKS = /* @__PURE__ */ new Set([169, 174, 8482]);
    BE_FORMS = ["am", "is", "are", "was", "were", "be", "been", "being"];
    IRREGULAR_PARTICIPLES = /* @__PURE__ */ new Set([
      "begun",
      "bent",
      "bound",
      "born",
    ...
    NOT_PARTICIPLES = /* @__PURE__ */ new Set([
    ...
    PASSIVE_MODIFIER = "(?:not|being|just|also|already|still|never|only|\\w+ly)\\s+";
    PASSIVE = new RegExp(
...
function loadVocabulary() {
  ...
  if (!pluginRoot) {
    fail(
      `Cannot read the vocabulary file: ${VOCABULARY_PATH} (no plugin root found above ${import.meta.dirname})`,
      VOCABULARY_HINT
    );
  }
  ...
  try {
    ...
    fail(`Cannot read the vocabulary file: ${path} (${err.message})`, VOCABULARY_HINT);
  }
...
function markdownIn(dir) {
  return import_fast_glob.default.sync("**/*.md", {
    cwd: dir,
    absolute: true,
    ignore: IGNORED,
    dot: false,
...
function collectFiles(paths, opts) {
  const files = [];
  const missing = [];
...
function summarize(reports) {
  ...
  const perFile = counted.filter((report) => report.findings.length > 0).map((report) => ({ path: report.path, count: report.findings.length })).sort((a, b) => b.count - a.count || a.path.localeCompare(b.path));
  return {
    perFile,
...
function ruleLabel(rule) {
  ...
  switch (rule) {
    ...
    case "mark":
      return source_default.magenta(padded);
    case "emoji":
...
function reportFile(file, display, vocabulary) {
  ...
  if (findings.length > 0) {
    console.log(source_default.bold(display));
    for (const finding of findings) {
      const where = `${finding.line}:${finding.column}`.padStart(9);
      console.log(`  ${source_default.dim(where)}  ${ruleLabel(finding.rule)} ${finding.message}`);
    }
    console.log("");
  }
  return { path: display, findings };
}
function printVocabularyWarnings(warnings, path) {
  if (warnings.length === 0) return;
  console.error(source_default.yellow(`${path}: ${warnings.length} row(s) dropped while parsing`));
  for (const warning of warnings) {
    console.error(source_default.yellow(`  line ${warning.line}: ${warning.message}`));
  }
  console.error("");
}
function printSummary(summary, missing) {
  console.log(source_default.bold("Summary"));
  if (missing.length > 0) {
    console.log(source_default.yellow(`  ${missing.length} path(s) not found`));
  }
  if (summary.perFile.length === 0) {
    console.log(source_default.green(`  clean: ${summary.totalFiles} file(s), no findings`));
    return;
  }
  for (const entry of summary.perFile) {
    console.log(`  ${source_default.yellow(String(entry.count).padStart(5))}  ${entry.path}`);
  }
  console.log(
...
function register8(program3) {
  program3.command("lint-prose").description("Warn about prose that drifts from the writing standard").argument("[paths...]", "files or directories to lint").option("--all", "lint every .md file in the repository").action((paths, opts) => {
    ...
    for (const path of missing) {
      console.error(source_default.yellow(`Not found, skipping: ${path}`));
    }
    ...
    for (const file of files) {
      ...
      if (isVocabularyFile(file, vocabulary.path)) {
        console.log(source_default.dim(`${display}: skipped, this is the vocabulary file itself
`));
...
var import_fast_glob, IGNORED, RULE_WIDTH, VOCABULARY_HINT;
var init_lint_prose = __esm({
  "src/commands/lint-prose.ts"() {
    "use strict";
    init_source();
    ...
    init_utils();
    init_prose();
    IGNORED = ["**/node_modules/**", "**/.git/**", "**/dist/**", "**/.claude/**"];
    RULE_WIDTH = Math.max(...RULES.map((rule) => rule.length)) + 2;
    VOCABULARY_HINT = "lint-prose reads its word and mark lists from the plugin, not from your project.";
  }
});
