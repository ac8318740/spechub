...
After displaying the config, scan for missing or incomplete settings. If you find gaps, use AskUserQuestion to offer to fix them. Collect all gaps into a single prompt.
...
2. **frontend configured but `frontend.browser.mode` is missing** – browser mode unknown
3. **frontend configured but `frontend.browser.cdp_port` is missing** – default `19988` applies for `mode: remote`, `9555` for `headless`/`local`, but not explicit
...
6. **No `frontend.helpers_dir`** (if frontend configured) – location of the verification knowledge base unknown

If you find gaps, show a single AskUserQuestion:
...
- **Set browser mode**: ask a follow-up AskUserQuestion with remote/headless/local options (same as the `check` command's browser connectivity section). Store in project.yaml. Walk through setup if the user chooses remote. If remote, also ask about fallback behavior (headless or none).
...
Audit the project for missing infrastructure. Offer to fix each issue. Run these checks in order:
...
If the user agrees, run Step 5 of the `init` skill. That step explores `directories.source`, proposes domains, confirms them, and writes the file.

If `spechub/specs/` already contains domain directories, propose those names first. Map paths to them. Renaming a domain here orphans its existing `spec.md`.
...
If the user agrees, run `npm install -g agent-browser`.
...
If the user agrees, write the file. Use `frontend.browser.cdp_port` from project.yaml; if unset, default to `19988` when `frontend.browser.mode` is `remote`, otherwise `9555`.

#### 5. Browser connectivity (if frontend configured)

Read `frontend.browser.cdp_port` from project.yaml, with the mode-aware default above. Use it as `<cdp_port>`:

```bash
curl -s --max-time 3 http://localhost:<cdp_port>/json/version
```

Report status. Then offer guidance:
...
If the user selects "Remote browser", set `frontend.browser.mode: remote` and `frontend.browser.cdp_port: 19988` in project.yaml. Update `agent-browser.json` to match. Walk through setup. Remote mode uses the Playwriter bridge. The Playwriter extension drives Chrome on the browser machine via its `chrome.debugger` API. The bridge opens no CDP listener on Chrome itself.
...
- Each tab needs the extension icon clicked once. The extension cannot attach to `chrome://` or `about:` pages.
...
For a persistent Windows laptop setup that opens no window – auto-reconnecting scheduled tasks, ssh-agent key persistence, one-time admin registration – see `plugins/spechub/docs/playwriter-bridge-windows.md`. It ships the three PowerShell scripts (`relay.ps1`, `tunnel.ps1`, `register-tasks.ps1`) under `plugins/spechub/assets/playwriter-bridge/`.

If the user selects "Headless", set `frontend.browser.mode: headless` in project.yaml. No further setup needed – the frontend-verifier launches Chromium automatically.

If the user selects "Skip for now", leave `frontend.browser.mode` unset.
...
Check whether the frontend-verifier can launch headless Chromium:
...
The nudge fires only on the session's own stop. It never nudges teammates or subagents, because neither can hand the user's work over.
...
- **show**: Display the formatted config summary. Then prompt to fix any gaps.
- **check**: Run health checks. Offer fixes interactively.
- **set**: Parse the key path. Validate the value. Update the YAML. Write it back.
- **reset**: Replace the `workflow` section with defaults. Preserve all other sections. Also preserve `workflow.maps`. It records the tracker choice the map skill persisted. Losing it strands node records on a backend no session looks at.
