...
Workflow:
  ...
  Handoff:       agent=claude, ack_turns=5, self_invoke=true, nudge_warn=200000, nudge_severe=500000
...
| `workflow.handoff.nudge_warn` | number | Absolute context tokens (not a percentage – there is no reliable way to learn a session's context-window size) at which the context-pressure hook starts nudging. Small-context models want lower values (default `200000`) |
| `workflow.handoff.nudge_severe` | number | Absolute context tokens at which the nudge escalates to severe (default `500000`) |
