...
Every watch reports the moment it started, as `startedAt` in epoch milliseconds,
and ignores any sidecar written before that cut-off. The default cut-off is the
watch's own start. So a sidecar from an earlier round never closes this watch,
and nothing has to delete `<handoff-file>.ack` between attempts.

The default needs no flag. `--ack-after <epoch-ms>` moves the cut-off back, for
a restart that has to reach behind its own start. Only the nudge restart does –
*Nudge once, then watch again*, below.
...
because that sidecar predates it.

Never pass `--ack-after` on such a relaunch. It aims at a new agent, so the
default cut-off is the one that keeps the old decline out.
